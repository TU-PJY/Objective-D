...
void Player::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity)
{
	if (dwDirection)
	{
		...
		if (dwDirection & DIR_FORWARD) xmf3Shift = Vec3::Add(xmf3Shift, m_xmf3Look, fDistance);
		if (dwDirection & DIR_BACKWARD) xmf3Shift = Vec3::Add(xmf3Shift, m_xmf3Look, -fDistance);
		if (dwDirection & DIR_RIGHT) xmf3Shift = Vec3::Add(xmf3Shift, m_xmf3Right, fDistance);
		if (dwDirection & DIR_LEFT) xmf3Shift = Vec3::Add(xmf3Shift, m_xmf3Right, -fDistance);
		if (dwDirection & DIR_UP) xmf3Shift = Vec3::Add(xmf3Shift, m_xmf3Up, fDistance);
		if (dwDirection & DIR_DOWN) xmf3Shift = Vec3::Add(xmf3Shift, m_xmf3Up, -fDistance);
...
void Player::Move(const XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	if (bUpdateVelocity)
	{
		m_xmf3Velocity = Vec3::Add(m_xmf3Velocity, xmf3Shift);
	}
	else
	{
		m_xmf3Position = Vec3::Add(m_xmf3Position, xmf3Shift);
...
void Player::Rotate(float x, float y, float z)
{
	...
	if ((nCurrentCameraMode == FIRST_PERSON_CAMERA) || (nCurrentCameraMode == THIRD_PERSON_CAMERA))
	{
		...
		if (y != 0.0f)
		{
			...
			m_xmf3Look = Vec3::TransformNormal(m_xmf3Look, xmmtxRotate);
			m_xmf3Right = Vec3::TransformNormal(m_xmf3Right, xmmtxRotate);
		}
	}
	else if (nCurrentCameraMode == SPACESHIP_CAMERA)
	{
		...
		if (x != 0.0f)
		{
			...
			m_xmf3Look = Vec3::TransformNormal(m_xmf3Look, xmmtxRotate);
			m_xmf3Up = Vec3::TransformNormal(m_xmf3Up, xmmtxRotate);
		}
		if (y != 0.0f)
		{
			XMMATRIX xmmtxRotate = XMMatrixRotationAxis(XMLoadFloat3(&m_xmf3Up), XMConvertToRadians(y));
...
{
	m_xmf3Velocity = Vec3::Add(m_xmf3Velocity, Vec3::ScalarProduct(m_xmf3Gravity, fTimeElapsed, false));
	...
	fLength = Vec3::Length(m_xmf3Velocity);
	float fDeceleration = (m_fFriction * fTimeElapsed);
	if (fDeceleration > fLength) fDeceleration = fLength;
	m_xmf3Velocity = Vec3::Add(m_xmf3Velocity, Vec3::ScalarProduct(m_xmf3Velocity, -fDeceleration, true));
}
...
Camera* Player::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	{
		m_xmf3Right = Vec3::Normalize(XMFLOAT3(m_xmf3Right.x, 0.0f, m_xmf3Right.z));
		m_xmf3Up = Vec3::Normalize(XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_xmf3Look = Vec3::Normalize(XMFLOAT3(m_xmf3Look.x, 0.0f, m_xmf3Look.z));

		m_fPitch = 0.0f;
		m_fRoll = 0.0f;
		m_fYaw = Vec3::Angle(XMFLOAT3(0.0f, 0.0f, 1.0f), m_xmf3Look);
...
Camera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	m_pCamera->SetPosition(Vec3::Add(m_xmf3Position, m_pCamera->GetOffset()));
	Update(fTimeElapsed);
