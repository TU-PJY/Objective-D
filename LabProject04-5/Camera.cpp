...
bool Camera::IsInFrustum(BoundingBox& xmBoundingBox)
{
#ifdef _WITH_DIERECTX_MATH_FRUSTUM
	return(m_xmFrustumWorld.Intersects(xmBoundingBox));
#else
...
{
	m_xmf4x4View = Mat4::LookAtLH(m_xmf3Position, m_xmf3LookAtWorld, m_xmf3Up);
}
...
void Camera::RegenerateViewMatrix()
{
	...
	m_xmf4x4View._41 = -Vec3::DotProduct(m_xmf3Position, m_xmf3Right);
	m_xmf4x4View._42 = -Vec3::DotProduct(m_xmf3Position, m_xmf3Up);
	m_xmf4x4View._43 = -Vec3::DotProduct(m_xmf3Position, m_xmf3Look);
...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		m_xmf3Position = Vec3::Subtract(m_xmf3Position, m_pPlayer->GetPosition());
		m_xmf3Position = Vec3::TransformCoord(m_xmf3Position, xmmtxRotate);
		m_xmf3Position = Vec3::Add(m_xmf3Position, m_pPlayer->GetPosition());
	}
	if (m_pPlayer && (y != 0.0f))
	{
...
CFirstPersonCamera::CFirstPersonCamera(Camera* pCamera) : Camera(pCamera)
{
	...
	if (pCamera)
	{
		if (pCamera->GetMode() == SPACESHIP_CAMERA)
		{
			...
			m_xmf3Right.y = 0.0f;
			m_xmf3Look.y = 0.0f;
			m_xmf3Right = Vec3::Normalize(m_xmf3Right);
			m_xmf3Look = Vec3::Normalize(m_xmf3Look);
		}
	}
}
...
void CThirdPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	if (m_pPlayer)
	{
		...
		XMFLOAT3 xmf3Offset = Vec3::TransformCoord(m_xmf3Offset, xmf4x4Rotate);
		XMFLOAT3 xmf3Position = Vec3::Add(m_pPlayer->GetPosition(), xmf3Offset);
		XMFLOAT3 xmf3Direction = Vec3::Subtract(xmf3Position, m_xmf3Position);
		...
		if (fDistance > 0)
		{
			m_xmf3Position = Vec3::Add(m_xmf3Position, xmf3Direction, fDistance);
...
{
	XMFLOAT4X4 mtxLookAt = Mat4::LookAtLH(m_xmf3Position, xmf3LookAt, m_pPlayer->GetUpVector());
